...
export class Card extends Component<ICard> {
  ...
  constructor(container: HTMLElement) {
    ...
    this.imageElement =
      this.container.querySelector<HTMLImageElement>(".card__image") ?? undefined;
    this.priceElement = ensureElement<HTMLElement>(
      ".card__price",
      this.container,
    );
    // Ищем элемент категории, если есть
  ...
  if (this.categoryElement) {
    ...
    const cssClass = (value in categoryMap ? categoryMap[value as keyof typeof categoryMap] : categoryMap['другое']);
